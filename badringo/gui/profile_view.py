"""
gui/profile_view.py
Profil pédagogique de l'apprenant (Phase 2 — Learner Profile).

Le prénom, la langue maternelle et l'objectif général sont saisissables
manuellement. Le niveau estimé, lui, ne l'est jamais : il sera calculé
automatiquement une fois le Skill Assessment (Phase 7) en place.
"""
...
class ProfileView(tk.Frame):
    ...
    def _build(self):
        ...
        carte_niveau.pack(fill="x", padx=20, pady=(0, 20))

        niveau_affiche = profil["niveau_estime"] if profil and profil["niveau_estime"] else "Non évalué pour l'instant"
        tk.Label(carte_niveau, text="Niveau estimé", bg="#f0f4ff", font=("Segoe UI", 10), fg="#555").pack(anchor="w")
        tk.Label(carte_niveau, text=niveau_affiche, bg="#f0f4ff", font=("Segoe UI", 18, "bold")).pack(anchor="w")
        tk.Label(
            carte_niveau,
            text="Calculé automatiquement à partir de tes résultats (bientôt disponible).",
